...
def validate_file_content_type(content_type: str, allowed_types: List[str]) -> ValidationResult:
    ...
    return ValidationResult(
        valid=False,
        errors=[f"Content type must be one of: {', '.join(allowed_types)}"]
    )


def validate_json_structure(data: Dict[str, Any], required_keys: List[str]) -> ValidationResult:
...
def validate_numeric_range(value: float, min_val: Optional[float] = None, max_val: Optional[float] = None) -> ValidationResult:
