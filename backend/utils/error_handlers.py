...
class RateLimitError(DeciseraException):
    ...
    def __init__(self, message: str = "Rate limit exceeded", retry_after: int = 60):
        super().__init__(
            message, status.HTTP_429_TOO_MANY_REQUESTS, {"retry_after": retry_after}
        )
...
def format_error_response(
    error: Exception, request: Optional[Request] = None, include_trace: bool = False
) -> Dict[str, Any]:
    """
...
async def handle_decisera_exception(
    request: Request, exc: DeciseraException
) -> JSONResponse:
