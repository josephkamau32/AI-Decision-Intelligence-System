...
class ForecastPlot:
    def __init__(
        self, df: pd.DataFrame, model, date_col: str, target_col: str, periods: int = 10
    ):
        self.df = df
        self.model = model
    ...
    def generate_plot(self) -> Dict[str, Any]:
        ...
        if hasattr(self.model, "predict"):  # sklearn-like
            ...
            future_dates = pd.date_range(
                start=last_date, periods=self.periods + 1, freq="D"
            )[1:]
            # Mock predictions
            predictions = (
                np.random.randn(self.periods) * df_sorted[self.target_col].std()
                + df_sorted[self.target_col].mean()
            )
            fig.add_trace(
                go.Scatter(
                    x=future_dates,
    ...
    def get_static_image(self) -> bytes:
        ...
        fig.add_trace(
            go.Scatter(
                x=future_dates, y=predictions, mode="lines+markers", name="Forecast"
            )
        )
