...
class TestFeatureEngineer:
    ...
    def test_encode_categorical_label_multiple_columns_persistence(self):
        ...
        recovered_colors = engineer.encoders['color'].inverse_transform(encoded_df['color'])
        recovered_sizes = engineer.encoders['size'].inverse_transform(encoded_df['size'])
    ...
    def test_select_features_missing_target_raises_value_error(self, sample_df):
        ...
        with pytest.raises(ValueError, match="Target column 'missing_target' not found"):
            engineer.select_features(target='missing_target', k=1, problem_type='classification')
