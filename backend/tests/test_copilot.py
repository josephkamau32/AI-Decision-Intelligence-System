...
class TestCopilotAgent:
    ...
    def test_query_success_with_mocked_llm(self):
        ...
        with patch("backend.copilot.agent.settings") as mock_settings, \
             patch("backend.copilot.agent.genai.configure") as mock_configure, \
    ...
    def test_query_handles_auth_error_gracefully(self):
        ...
        with patch("backend.copilot.agent.settings") as mock_settings, \
             patch("backend.copilot.agent.genai.configure"), \
             patch("backend.copilot.agent.genai.GenerativeModel") as mock_model_cls:
            ...
            mock_instance.generate_content.side_effect = UnauthenticatedError("API key not valid. Please pass a valid API key.")
            mock_model_cls.return_value = mock_instance
    ...
    def test_query_handles_quota_error_gracefully(self):
        ...
        with patch("backend.copilot.agent.settings") as mock_settings, \
            ...
            mock_instance.generate_content.side_effect = ResourceExhausted("ResourceExhausted: Quota exceeded")
            mock_model_cls.return_value = mock_instance
    ...
    def test_query_handles_model_not_found_gracefully(self):
        ...
        with patch("backend.copilot.agent.settings") as mock_settings, \
            ...
            mock_instance.generate_content.side_effect = NotFoundError("Model not found")
            mock_model_cls.return_value = mock_instance
