...
async def initialize_model_monitoring(
    model_id: str, reference_data: dict, problem_type: str = "classification"
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to initialize monitoring: {str(e)}"
        )
...
async def update_performance_baseline(model_id: str, y_true: list, y_pred: list):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update baseline: {str(e)}"
        )
...
async def monitor_performance(model_id: str, y_true: list, y_pred: list):
    ...
    try:
        ...
        y_true_arr = np.array(y_true)
        y_pred_arr = np.array(y_pred)
        result = monitoring_service.monitor_performance(
            model_id, y_true_arr, y_pred_arr
        )
        return result
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to monitor performance: {str(e)}"
        )
