...
class DataProfiler:
    ...
    def detect_column_types(self) -> Dict[str, str]:
        ...
        for col in self.df.columns:
            ...
            if pd.api.types.is_datetime64_any_dtype(col_data):
                ...
                try:
                    pd.to_datetime(
                        col_data.dropna().head(100), errors="raise", format="mixed"
                    )
    ...
    def generate_profile(self) -> Dict[str, Any]:
        ...
        for col in self.df.columns:
            ...
            col_profile = {
                ...
                "missing_percent": (
                    float(col_data.isna().sum() / len(col_data) * 100)
                    if len(col_data) > 0
                    else 0.0
                ),
            ...
            if pd.api.types.is_numeric_dtype(col_data):
                col_profile["statistics"] = {
                    "mean": (
                        float(col_data.mean()) if not col_data.isna().all() else None
                    ),
                    "std": float(col_data.std()) if not col_data.isna().all() else None,
                    "min": float(col_data.min()) if not col_data.isna().all() else None,
                    "max": float(col_data.max()) if not col_data.isna().all() else None,
                    "median": (
                        float(col_data.median()) if not col_data.isna().all() else None
                    ),
                    "q25": (
                ...
                col_profile["outliers_percent"] = (
                    float(outlier_mask.sum() / len(col_data) * 100)
                    if len(col_data) > 0
                    else 0.0
                )
    ...
    def get_data_quality_issues(self) -> List[Dict[str, Any]]:
        ...
        for col_profile in self.profile_data.get("columns_info", []):
            ...
            if (
                "outliers_percent" in col_profile
                and col_profile["outliers_percent"] > 10
            ):
                issues.append(
                    {
                        "severity": "medium",
        ...
        if (
            self.profile_data.get("duplicates", 0)
            / max(self.profile_data.get("rows", 1), 1)
            > 0.1
        ):
            issues.append(
                {
...
def ingest_and_profile_dataset(file_path: str) -> Dict[str, Any]:
    ...
    return {"dataframe": df, "profile": profile, "quality_issues": issues}
