        ...
        if strategy == "drop":
            ...
            if num_cols.any():
                num_imputer = SimpleImputer(
                    strategy=strategy if strategy in ["mean", "median"] else "mean"
                )
...
class FeatureEngineer:
    ...
    def encode_categorical(
        self, method: str = "onehot", drop_first: bool = True
    ) -> pd.DataFrame:
        logger.info(f"Encoding categorical variables with method: {method}")
        cat_cols = self.df.select_dtypes(include=["object", "category"]).columns
        if method == "onehot":
            self.df = pd.get_dummies(
                self.df, columns=cat_cols, drop_first=drop_first, dtype=int
            )
    ...
    def select_features(
        self, target: str, k: int = 10, problem_type: str = "regression"
    ) -> pd.DataFrame:
